...
module.exports = function (pool) {
  ...
  // Simple function to get userId from JWT token
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      console.log("🔍 Token present:", !!tokenFromQuery);
      ...
      const userId = getUserIdFromToken(tokenFromQuery);
      console.log("🔍 UserId:", userId);

      if (!userId) {
        return res.status(401).send("Please log in to connect your store.");
      }
  ...
  router.get("/callback", async (req, res) => {
    try {
      ...
      // Save to shops table (backwards compatibility)
      await pool.query(
